...
class Agent:
    ...
    def eval(self, eps: int, logname: str = "eval"):
        ...
        ep_frames = sum(logs["num_frames_per_episode"])
        fps = ep_frames / (end_time - start_time)
        ...
        header = ["trained_frames", "ep_frames", "FPS", "duration"]
        data = [status['num_frames'], ep_frames, fps, duration]
        ...
        print("F {} | FPS {:.0f} | D {} | R:μσmM {:.2f} {:.2f} {:.2f} {:.2f} | F:μσmM {:.1f} {:.1f} {} {}"
                .format(ep_frames, fps, duration,
                        *return_per_episode.values(),
                        *num_frames_per_episode.values()))
        
    def train(self, frames: int, algo_type: Literal['ppo', 'a2c'], algo_config: Config = Config(), log_interval: int = 1, save_interval: int = 1, logname: str = "train", save_on_completion: bool = True) -> bool:
        txt_logger = utils.get_txt_logger(self.model_dir, logname)
        csv_file, csv_logger = utils.get_csv_logger(self.model_dir, logname)
        tb_writer = tensorboardX.SummaryWriter(self.model_dir+"/tb")

        try:
            status = utils.get_status(self.model_dir)
        except OSError:
            status = {"num_frames": 0, "update": 0}
        print(status['num_frames'], "frames")
